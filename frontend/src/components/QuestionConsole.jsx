...
function QuestionConsole({ island, onSubmitPreRound, onSubmitAnswer, loading, isCompleted, onNextIsland, preRoundQuestion, mainQuestion, shakeError, eliminatedOptions }) {
  ...
  return (
    <div className="question-console-grid">
      {preRoundQuestion && (
        <section className={`surface-panel console-card question-marker cinematic-panel ${shakeError ? 'shake-animation' : ''}`}>
              ...
              <div className="field">
                ...
                >
                  ...
                  {preRoundQuestion.options.map((opt, i) => {
                    const isEliminated = eliminatedOptions?.[preRoundQuestion.id] === opt;
                    return (
                      <option key={i} value={opt} disabled={isEliminated}>
                        {opt} {isEliminated ? '(Eliminated)' : ''}
                      </option>
                    );
                  })}
                </select>
                  ...
                  <div className="field">
                    ...
                    >
                      ...
                      {mainQuestion.options.map((opt, i) => {
                        const isEliminated = eliminatedOptions?.[mainQuestion.id] === opt;
                        return (
                          <option key={i} value={opt} disabled={isEliminated}>
                            {opt} {isEliminated ? '(Eliminated)' : ''}
                          </option>
